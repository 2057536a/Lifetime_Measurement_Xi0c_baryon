...
hist2 = ROOT.TH1F('hist2','Signal values in decay time intervals', 10, 0.0002, 0.001)


###LOOPING OVER DECAY TIME INTERVALS###

#initial_data = data
Xc_TAU_lower_limit = 0.0002      #units in ns (0.1 ps)
...
while Xc_TAU_lower_limit < Xc_TAU_upper_limit:
    ...
    second_data = first_data.reduce('Xc_TAU <= ' + str(Xc_TAU_lower_limit + 0.00008))
    ...
    hist2.SetBinContent(bin_index,signal_number)
    hist2.SetBinError(bin_index,error_signal)
          
    print "For Xc_TAU > "+str(Xc_TAU_lower_limit) + " and" " Xc_TAU < "+ str(Xc_TAU_lower_limit + 0.00008) + ' the results are: ' 
    ...
    Xc_TAU_lower_limit += 0.00008
                         
    bin_index += 1


myhistogram2 = ROOT.TFile('myhist2.root','recreate')        #creates a new myhist2.root file
hist2.Write()                                               # writes the histogram into the root file so it can be retrieved later for RooDataHist
