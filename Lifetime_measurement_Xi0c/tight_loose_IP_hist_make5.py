...
data = ROOT.RooDataSet('data','dataset from tree', ROOT.RooArgSet(mass,bdt,xc_tau,xc_ipchi2), ROOT.RooFit.Import(datatree),ROOT.RooFit.Cut('BDT > 0.01 && Xc_IPCHI2_OWNPV < 1'))
...
hist7 = ROOT.TH1F('hist7','hist7', 9, 0.0001, 0.001)
...
while Xc_TAU_lower_limit < Xc_TAU_upper_limit:
    ...
    hist7.SetBinContent(bin_index,signal_number)
    hist7.SetBinError(bin_index,error_signal)
          
    print "For Xc_TAU > "+str(Xc_TAU_lower_limit) + " and" " Xc_TAU < "+ str(Xc_TAU_lower_limit + 0.0001) + ' the results are: ' 
    ...
    Xc_TAU_lower_limit += 0.0001
                         
    bin_index += 1


myhistogram7 = ROOT.TFile('myhist7.root','recreate')        #creates a new myhist.root file
hist7.Write()                                             # writes the histogram into the root file so it can be retrieved later for RooDataHist
